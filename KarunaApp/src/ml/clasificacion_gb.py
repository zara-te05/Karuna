...
def entrenar(X,y,Xtr,Xte,ytr,yte,loo,k_folds=None):
    ...
    if loo:
        ...
        try:
            ...
            sc=np.array([])
        m.fit(X,y)  # refit final — solo para graficar importancias
        return {"model":m,"scores_loo":sc,"y_pred":m.predict(X),"y_prob":m.predict_proba(X)[:,1],
                "y_test":y,"X_test":X, "acc_from_cv": True}
    m.fit(Xtr,ytr)
    return {"model":m,"scores_loo":None,"y_pred":m.predict(Xte),"y_prob":m.predict_proba(Xte)[:,1],
            "y_test":yte,"X_test":Xte, "acc_from_cv": False}
