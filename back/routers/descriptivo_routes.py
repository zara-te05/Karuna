...
from repositories.promedios_repo import obtener_promedios_parcial, obtener_valores_calculo_parcial, obtener_promedio_final_alumno , obtener_cantidad_parciales
...
@router.get("/alumno-final/{alumno_id}/grupo/{grupo_periodo_id}")
async def promedio_final_alumno(alumno_id : int, grupo_periodo_id: int):
    
    valores = obtener_promedio_final_alumno(alumno_id, grupo_periodo_id)
    
    if not valores:
        raise HTTPException(status_code=404, detail='No hay valores')
    
    cantidad_parciales = obtener_cantidad_parciales(grupo_periodo_id)
 
    promedio_final = Descriptivos.promedio_final_alumno(
        valores,
        cantidad_parciales
    )
